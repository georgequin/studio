...
/**
 * @fileOverview This file defines a Genkit flow for categorizing news articles related to human rights violations.
 *
 * - categorizeNewsClipping - A function that takes news article text as input and returns a categorization of the content.
 * - CategorizeNewsClippingInput - The input type for the categorizeNewsClipping function, which is the text content of the news article.
...
const CategorizeNewsClippingInputSchema = z.object({
  text: z
    .string()
    .describe('The text content of the news article to be categorized.'),
});
...
const CategorizeNewsClippingOutputSchema = z.object({
  category: z
    .string()
    .describe(
      'The determined category for the news article, chosen from predefined NHRC categories such as Communal Clashes, Terrorism, Women & Children, etc.'
    ),
  confidence: z
    .number()
    .describe(
...
const categoryPrompt = ai.definePrompt({
  ...
  Analyze the following news article and determine the most appropriate category. Also, provide a confidence level (0-1) for your categorization.
